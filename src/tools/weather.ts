...
export const getForecastTool = {
  definition: {
    name: "get_forecast",
    description:
      "Get a weather forecast for the given coordinates (default 3 days). Returns min and max daily temperatures.",
    inputSchema: {
   ...
   */
  handler: async (args: GetForecastArgs): Promise<CallToolResult> => {
    try {
      const { latitude, longitude, days = 3 } = args;

      // Fetch data from the weather API
      const data: WeatherForecastResponse = await weatherClient.getForecast(
        latitude,
        longitude,
        days
      );
